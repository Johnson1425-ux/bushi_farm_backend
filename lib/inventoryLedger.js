...
/* Money and quantities both need this, and `toFixed` hands back a string
   that then has to be parsed again at every call site. */
const round = (v, dp = 2) => {
  const f = 10 ** dp;
...
/* ══════════════════════════════════════════════════════════════
   PACKS AND WHAT A DOSE COSTS

   Plenty of what the store holds is bought in a container and used out
   of it a little at a time: a 100 ml bottle of oxytetracycline, a 25 kg
   sack of dairy meal, a 20 litre drum of CIP caustic. The bottle is what
   the invoice prices — 9,000 — and the millilitre is what the vet draws.

   Holding stock in whole bottles cannot express that. Twenty millilitres
   out of a hundred is not a fifth of a bottle you can take off the shelf
   and put back; it is a bottle that is now four fifths full. So the
   ledger holds the BASE unit — millilitres — and an item records how
   many of them are in one pack.

     unit        the base unit, the one stock moves in        ml
     pack_unit   what it is bought and priced in              bottle
     pack_size   base units in one pack                       100
     unit_cost   cost of one base unit                        90
     pack cost   pack_size × unit_cost                        9,000

   Nobody types 90. They type the 9,000 on the invoice and the pack size
   off the label, and the cost per millilitre falls out of the two. Draw
   20 ml and it costs 1,800, because 20 × 90 is what that arithmetic was
   for.
══════════════════════════════════════════════════════════════ */

/** Base units in one pack — never zero, so it is always safe to divide by. */
const packSize = (item) => {
  const n = num(item?.pack_size);
  return n > 0 ? n : 1;
};

/** What one whole pack costs: the figure on the invoice. */
const packCost = (item) => round(packSize(item) * num(item?.unit_cost), 2);

/** A quantity of base units expressed in packs, e.g. 480 ml → 4.8 bottles. */
const toPacks = (quantity, item) => round(num(quantity) / packSize(item), 3);

/** Packs to base units, e.g. 5 bottles → 500 ml. */
const toBase = (packs, item) => round(num(packs) * packSize(item), 3);

/**
 * What a movement of this size is worth.
 *
 * Deliberately takes the cost per base unit as an argument rather than
 * reading it off the item: an outgoing movement is costed at the average
 * that stood when it happened, and that figure is stamped on the row. Go
 * back to the item later and you would be pricing last month's doses at
 * this month's price, which is how a treatment quietly changes what it
 * cost after the fact.
 */
const lineCost = (quantity, unitCost) => round(Math.abs(num(quantity)) * num(unitCost), 2);

/**
 * The item's cost per base unit after a delivery lands.
 *
 * A moving weighted average, which is what a store ledger normally keeps
 * and the only honest answer once two deliveries at different prices are
 * mixed in the same drum. Bottles bought at 9,000 and then at 10,000 make
 * every millilitre drawn afterwards worth something between the two —
 * pricing them all at the newest invoice would overstate the cost of
 * doses that came out of the older stock.
 *
 * Two cases take the new price outright: an empty shelf, where there is
 * nothing to average against, and stock that was never priced at all,
 * where averaging would treat what is already there as free and drag the
 * figure below anything the farm ever paid.
 */
function averageCost({ onHand, currentCost, receivedQty, receivedCost }) {
  const have = num(onHand);
  const old  = num(currentCost);
  const qty  = num(receivedQty);
  const paid = num(receivedCost);

  if (!(paid > 0)) return old;            // an unpriced delivery says nothing about cost
  if (have <= 0 || old <= 0) return paid;
  return round((have * old + qty * paid) / (have + qty), 6);
}
