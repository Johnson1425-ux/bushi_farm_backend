...
/* ─── ANIMAL IDENTIFICATION AND HISTORY ───────────────────────
   ...
   Dam and sire are the farm's own addition, not on the printed sheet —
   they sit beside breed because that is what they qualify. Adding them
   here is what puts them on the in-app form, into the blank .docx, and in
   front of the parser, so a sheet already in circulation without them
   still reads back fine: an absent label is simply a field nobody
   answered. */
const IDENTIFICATION = [
  { key: 'cow_tag',          label: 'ID/Tag no.',   aliases: ['Cow ID/Tag', 'Cow ID', 'ID/Tag'] },
  { key: 'age',              label: 'Age' },
  { key: 'breed',            label: 'Breed' },
  { key: 'dam',              label: 'Dam',          aliases: ['Dam (mother)', 'Mother'] },
  { key: 'sire',             label: 'Sire',         aliases: ['Sire (father)', 'Father'] },
  { key: 'sex',              label: 'sex' },
