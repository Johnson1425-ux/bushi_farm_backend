...
function parsePayroll(ws, sheetName, period, issues) {
  ...
  /* "TOTAL EARNINGS" is what was paid; GROSS SALARY is before an advance
     is taken off, and posting that would count a loan the farm has
     already handed over a second time. The header can sit on either of
     the two banner rows a payroll is printed with. */
  let payCol = null;
  for (let r = headerRow; r <= headerRow + 1 && payCol === null; r++) {
    for (let c = 0; c < width; c++) {
      if (norm(rows[r]?.[c]) === 'TOTALEARNINGS') { payCol = c; break; }
    }
  }
  if (payCol === null) {
    issues.warnings.push(`${sheetName}: the payroll has no "TOTAL EARNINGS" column, so no salaries were imported.`);
    return [];
  }
  ...
  if (!entries.length) {
    issues.warnings.push(
      `${sheetName}: a payroll was found, but no employee on it has anything under `
      + '"TOTAL EARNINGS", so no salaries were imported.'
    );
  }
